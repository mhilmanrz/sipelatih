<x-layouts.app>
    ...
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-white">MANAJEMEN JENIS KEGIATAN</h1>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <x-table-toolbar actionUrl="{{ route('activity-types.index') }}" searchPlaceholder="Cari jenis kegiatan..." />
        ...
        <x-table>
            ...
            <tr class="border-b border-gray-200 hover:bg-gray-50">
                <td class="px-4 py-3 text-sm text-gray-500">{{ $activityTypes->firstItem() + $index }}</td>
                <td class="px-4 py-3 text-sm text-gray-900">{{ $activityType->name }}</td>
                <td class="px-4 py-3">
            ...
            @empty
            <tr>
                <td colspan="3" class="px-4 py-8 text-center text-gray-500">Belum ada data Jenis Kegiatan.</td>
            </tr>
            @endforelse
        </x-table>

        <x-table-footer :paginator="$activityTypes->appends(request()->query())" />
    </div>
