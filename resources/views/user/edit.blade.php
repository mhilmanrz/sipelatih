...
<style>
    ...
    .form-group label { display: block; margin-bottom: 5px; color: #007A7F; font-weight: 500;}
    .form-group input, .form-group select { width: 100%; padding: 10px; border: 1px solid #ccc; border-radius: 5px; }
    .btn-save { background: #00B8A5; color: white; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; }
    .btn-reset { background: #ccc; color: black; border: none; padding: 10px 20px; border-radius: 5px; cursor: pointer; text-decoration: none;}
</style>
@endpush
        ...
        <div class="form-group">
            ...
            <select name="work_unit_id">
                ...
                @foreach($workUnits as $wu)
                    <option value="{{ $wu->id }}" {{ (old('work_unit_id', $user->work_unit_id) == $wu->id) ? 'selected' : '' }}>{{ $wu->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="profession_id">
                ...
                @foreach($professions as $prof)
                    <option value="{{ $prof->id }}" {{ (old('profession_id', $user->profession_id) == $prof->id) ? 'selected' : '' }}>{{ $prof->name }}</option>
                @endforeach
            </select>
        </div>
        ...
        <div class="form-group">
            ...
            <select name="employment_type_id">
                ...
                @foreach($employmentTypes as $et)
                    <option value="{{ $et->id }}" {{ (old('employment_type_id', $user->employment_type_id) == $et->id) ? 'selected' : '' }}>{{ $et->name }}</option>
                @endforeach
            </select>
        </div>

        <hr style="margin: 30px 0;">
        <p style="text-align:center; color:#666; font-size:12px;">Abaikan form di bawah jika tidak ingin mengganti password</p>
