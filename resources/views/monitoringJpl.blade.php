...
<div class="p-6 space-y-6">
    ...
    <!-- FILTER -->
    <form action="{{ route('monitoring.jpl.index') }}" method="GET" class="bg-white p-4 rounded shadow flex flex-col md:flex-row gap-4 md:items-center">
        ...
        <a href="{{ route('monitoring.jpl.index') }}" class="bg-red-500 hover:bg-red-600 text-white px-6 py-2 rounded w-full md:w-auto text-center" style="text-decoration: none;">
            Reset
        </a>
    </form>
            ...
            <tbody>
                @forelse($users as $index => $user)
                    <tr class="border-b hover:bg-gray-50">
                        ...
                        <td class="px-3 py-2 text-center font-bold {{ $user->capaian_jpl >= $user->target_jpl ? 'text-green-600' : 'text-orange-500' }}">
            ...
            <tbody class="bg-gray-50">
                @forelse($detailedActivities as $index => $participant)
                    <tr class="border-b">
                        ...
                        <td class="px-4 py-3 text-teal-700 font-medium whitespace-nowrap">{{ $participant->user->name }}</td>
                        ...
                        <td class="px-4 py-3 font-medium min-w-[12rem]">{{ $participant->activity->activityName->name ?? ($participant->activity->name ?? 'N/A') }}</td>
                        ...
                        <td class="px-4 py-3 text-center font-bold text-teal-600">{{ number_format($participant->capaian_jpl, 1) }}</td>
