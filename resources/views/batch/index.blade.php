<x-layouts.app>
    <x-slot:title>Manajemen Batch</x-slot>

    <div class="px-8 py-6">

        {{-- TITLE & BUTTONS --}}
        <div class="flex flex-wrap justify-between items-center gap-4 mb-6">
            <x-page-title>Manajemen Batch</x-page-title>
                ...
                <table class="w-full text-sm border-collapse" style="min-width: 400px;">
                    <thead class="bg-[#007a7a] text-white">
                        <tr>
                            ...
                            <th class="text-left py-3 px-4 font-semibold text-sm">Nama Batch</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($batches as $index => $batch)
                            <tr class="hover:bg-gray-50 transition">
                                <td class="py-3 px-4">{{ $batches->firstItem() + $index }}</td>
                                <td class="py-3 px-4 text-left">{{ $batch->code ?? '-' }}</td>
                                <td class="py-3 px-4 font-medium">{{ $batch->name }}</td>
