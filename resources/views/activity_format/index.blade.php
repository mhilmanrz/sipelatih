<x-layouts.app>
    ...
    <div class="flex justify-between items-center mb-6">
        <h1 class="text-2xl font-bold text-white">MANAJEMEN BENTUK KEGIATAN</h1>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <x-table-toolbar actionUrl="{{ route('activity-formats.index') }}" searchPlaceholder="Cari bentuk kegiatan..." />

        <x-table>
            <x-slot name="header">
                <tr>
                    <th class="px-4 py-3">No.</th>
                    <th class="px-4 py-3">Bentuk Kegiatan</th>
            ...
            <tr class="border-b border-gray-200 hover:bg-gray-50">
                <td class="px-4 py-3 text-sm text-gray-500">{{ $activityFormats->firstItem() + $index }}</td>
                <td class="px-4 py-3 text-sm text-gray-900">{{ $activityFormat->name }}</td>
                <td class="px-4 py-3">
                    <div class="flex items-center gap-2">
                        ...
                        <form action="{{ route('activity-formats.destroy', $activityFormat->id) }}" method="POST"
                            class="inline m-0"
                            onsubmit="return confirm('Apakah Anda yakin ingin menghapus Bentuk Kegiatan ini?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
            ...
            @empty
            <tr>
                <td colspan="3" class="px-4 py-8 text-center text-gray-500">Belum ada data Bentuk Kegiatan.</td>
            </tr>
            @endforelse
        </x-table>

        <x-table-footer :paginator="$activityFormats->appends(request()->query())" />
    </div>
