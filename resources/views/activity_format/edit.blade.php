<x-layouts.app>
    <x-slot:title>Edit Bentuk Kegiatan</x-slot>
    ...
    <div class="tw-wrap p-6 max-w-2xl mx-auto">
        <div class="mb-6">
            <h1 class="text-2xl font-bold text-gray-800">Edit Data Bentuk Kegiatan</h1>
            <p class="text-gray-600 mt-1">Silakan perbarui form di bawah ini untuk mengubah data bentuk kegiatan.</p>
        </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            <form action="{{ route('activity-formats.update', $activityFormat->id) }}" method="POST">
                @csrf
                @method('PUT')

                <div class="mb-4">
                    <label for="code" class="block text-gray-700 font-semibold mb-2">Kode</label>
                    <input type="text" name="code" id="code" value="{{ old('code', $activityFormat->code) }}"
                ...
                <div class="mb-4">
                    <label for="name" class="block text-gray-700 font-semibold mb-2">Nama Bentuk Kegiatan <span
                            class="text-red-500">*</span></label>
                    <input type="text" name="name" id="name" value="{{ old('name', $activityFormat->name) }}"
                        ...
                        placeholder="Contoh: Klasikal" required>
                    @error('name')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
                </div>
