<x-layouts.app>
    <x-slot:title>Manajemen Permission</x-slot>

    <div class="px-8 py-6">

        {{-- TITLE & BUTTONS --}}
        <div class="flex flex-wrap justify-between items-center gap-4 mb-6">
            <x-page-title>Manajemen Permission</x-page-title>
        ...
        <form method="GET" action="{{ route('permissions.index') }}"
            ...
            <div class="flex flex-wrap items-center gap-3 px-5 py-4 border-b border-gray-200">
                ...
                <div class="flex items-center gap-2">
                    <input type="text" name="q" value="{{ request('q') }}" placeholder="Cari Nama Permission..."
                ...
                <table class="w-full text-sm border-collapse" style="min-width: 400px;">
                    <thead class="bg-[#007a7a] text-white">
                        <tr>
                            ...
                            <th class="text-left py-3 px-4 font-semibold text-sm">Nama Permission</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($permissions as $index => $permission)
                            <tr class="hover:bg-gray-50 transition">
                                <td class="text-center py-3 px-4">{{ $permissions->firstItem() + $index }}</td>
                                ...
                                <td class="text-center py-3 px-4">
                                    <div class="flex justify-center gap-1.5">
                                        ...
                                        <form action="{{ route('permissions.destroy', $permission->id) }}" method="POST" class="inline-block m-0">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit"
