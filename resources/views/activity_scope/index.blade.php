<x-layouts.app>
    @section('title', 'Manajemen Ruang Lingkup Kegiatan')
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        ...
        <x-table-toolbar actionUrl="{{ route('activity-scopes.index') }}" searchPlaceholder="Cari ruang lingkup..." />

        <x-table>
            <x-slot name="header">
                <tr>
                    <th class="px-4 py-3">No.</th>
                    <th class="px-4 py-3">Ruang Lingkup</th>
            ...
            <tr class="border-b border-gray-200 hover:bg-gray-50">
                <td class="px-4 py-3 text-sm text-gray-500">{{ $activityScopes->firstItem() + $index }}</td>
                <td class="px-4 py-3 text-sm text-gray-900">{{ $activityScope->name }}</td>
                <td class="px-4 py-3">
                    <div class="flex items-center gap-2">
                        ...
                        <form action="{{ route('activity-scopes.destroy', $activityScope->id) }}" method="POST"
                            class="inline m-0"
                            onsubmit="return confirm('Apakah Anda yakin ingin menghapus Ruang Lingkup ini?');">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
            ...
            @empty
            <tr>
                <td colspan="3" class="px-4 py-8 text-center text-gray-500">Belum ada data Ruang Lingkup.</td>
            </tr>
            @endforelse
        </x-table>

        <x-table-footer :paginator="$activityScopes->appends(request()->query())" />
    </div>
