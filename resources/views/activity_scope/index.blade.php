<x-layouts.app>
    ...
    <div class="px-8 py-6">
        <div class="flex flex-wrap justify-between items-center gap-4 mb-6">
            <x-page-title>Manajemen Ruang Lingkup Kegiatan</x-page-title>
        ...
        <div class="bg-white rounded-xl shadow-sm overflow-hidden mb-6">
                ...
                <table class="w-full text-sm border-collapse" style="min-width: 400px;">
                    <thead class="bg-[#007a7a] text-white">
                        <tr>
                            ...
                            <th class="text-left py-3 px-4 font-semibold text-sm">Ruang Lingkup</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($activityScopes as $index => $activityScope)
                            <tr class="hover:bg-gray-50 transition">
                                <td class="text-center py-3 px-4">{{ $activityScopes->firstItem() + $index }}</td>
                                <td class="py-3 px-4 text-left">{{ $activityScope->code ?? '-' }}</td>
                                <td class="font-medium py-3 px-4">{{ $activityScope->name }}</td>
            ...
            <div class="px-5 py-4 border-t border-gray-200">
                {{ $activityScopes->links('components.pagination') }}
            </div>
        </div>
    </div>
</x-layouts.app>
