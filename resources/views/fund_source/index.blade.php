...
<div class="tw-wrap p-6">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <!-- Table Control Bar -->
        <form method="GET" action="{{ route('fund-sources.index') }}"
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="whitespace-nowrap text-sm font-medium text-center space-x-2 border border-gray-200 py-3 px-4">
                            ...
                            <form action="{{ route('fund-sources.destroy', $fs->id) }}" method="POST" class="inline-block">
                                @csrf
                                @method('DELETE')
                                <button type="submit"

                                    onclick="return confirm('Apakah Anda yakin ingin menghapus sumber dana ini?')" style="background-color: #ef4444;" class="text-white px-3 py-1.5 rounded hover:bg-[#dc2626] text-sm font-semibold transition inline-block">
                                    Hapus
                                </button>
                            </form>
                        </td>
